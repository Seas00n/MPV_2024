...
def align_fea(pcd_new: pcd_opreator_system,
    ...
    if pcd_new.fea_extra_over and pcd_pre.fea_extra_over and pcd_new.env_type > 0 and pcd_pre.env_type > 0:
        if pcd_new.is_fea_C_gotten and pcd_pre.is_fea_A_gotten:
            if abs(pcd_new.Ccenter[0] - pcd_pre.Acenter[0]) < 0.05:
        ...
        elif pcd_new.is_fea_A_gotten and pcd_pre.is_fea_C_gotten:
            if abs(pcd_new.Acenter[0] - pcd_pre.Ccenter[0]) < 0.05:
        ...
        #         pcd_align_flag = 0
        #         return pcd_to_align_new, pcd_to_align_pre, pcd_align_flag
        if (pcd_new.is_fea_A_gotten and pcd_pre.is_fea_A_gotten) and (
                abs(pcd_new.Acenter[0] - pcd_pre.Acenter[0]) < 0.1):
...
def icp_knn(pcd_s, pcd_t, max_iterate=20):
